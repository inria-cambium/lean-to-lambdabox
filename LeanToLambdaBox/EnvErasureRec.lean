...
## Where Parts 1b and 2 went (slice Γ-W2)

`Erases.instFixvars`, `erases_target_fvars`, `erases_fix_of_closed` and the
`erases_fix_of_open{,_nil}` pair now live in **`RecBlockErasure.lean`**, unchanged. They
are the machinery the bridge's step 6 has to call in order to walk `visitMutual`'s
recursive exit, and this file is *downstream* of the bridge — through
`EnvErasureNonrec`, which this file imports at line 1 for Part 3's `RegisteredClosure`
analogy and for nothing else. Their own proof cone never touched that import; they were
misplaced, not entangled. What stays here is Part 3 (the registration certificate and its
demotion), its guards — which read the moved theorems back through the import chain — and
Part 3b's historical record.
-/
