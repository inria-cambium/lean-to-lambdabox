...
theorem shipping_visitExpr_correct'
    ...
    (hnfv : Γ.fixvars = fun _ => none)
    {gw : Void IO.RealWorld → NameGenerator}
    (R : ResidualHyps env₀ ves Us Γ gw) (HD : DataBridgeHyps Γ gw) (C : CasesBridgeHyps Γ gw) (P : ProjBridgeHyps Γ gw)
    (Hδ : ∀ (cc : Core.Context) (rf : ST.Ref IO.RealWorld Core.State),
  ...
  shipping_visitExpr_correct wf.tr.wf hΔ hcon hdelta hrec hnfv (R.toBridgeHyps wf) HD C P Hδ
...
example (Γ : ErasureCtx) (hΓrec : Γ.recBodies = fun _ => none)
    ...
    (gw : Void IO.RealWorld → NameGenerator)
    (H : BridgeHyps .empty [] Γ gw) (HD : DataBridgeHyps Γ gw) (C : CasesBridgeHyps Γ gw) (P : ProjBridgeHyps Γ gw)
    (Hδ : ∀ (cc : Core.Context) (rf : ST.Ref IO.RealWorld Core.State),
